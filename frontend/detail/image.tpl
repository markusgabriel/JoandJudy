...
{block name="frontend_detail_image"}
                                    ...
                                    <span class="image--media">
                                        {if isset($sArticle.image.thumbnails)}
                                            {block name='frontend_detail_image_default_picture_element'}
                                                <img srcset="{$sArticle.image.thumbnails[3].sourceSet}" alt="{$alt}" itemprop="image" />
                                            {/block}
                                        {else}
								                        ...
								                        <span class="image--media">
								                            {if isset($image.thumbnails)}
								                                {block name='frontend_detail_images_picture_element'}
								                                    <img srcset="{$image.thumbnails[3].sourceSet}" alt="{$alt}" itemprop="image" />
								                                {/block}
								                            {else}
    ...
    {* {foreach $sArticle.images as $image}
    {block name='frontend_detail_images_no_slider'}
        <div class="image--box no-slider">

            {block name='frontend_detail_images_no_slider_image_element'}
